...
class AccountsTest(TestCase):
    ...
    def test_register_user(self):
        data = {'username': 'newuser', 'email': 'u@example.com', 'password': 'newpass123'}
    ...
    def test_verify_and_password_reset_flow(self):
        # register user
        data = {'username': 'vuser', 'email': 'v@example.com', 'password': 'verify123'}
        resp = self.client.post(reverse('user-register'), data, format='json')
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED)
        ...
        resp = self.client.post(reverse('password-reset'), {'email': 'v@example.com'}, format='json')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertIn('token', resp.data)
        reset_token = resp.data['token']

        # confirm reset
        resp = self.client.post(reverse('password-reset-confirm'), {'token': reset_token, 'password': 'newpass456'}, format='json')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
